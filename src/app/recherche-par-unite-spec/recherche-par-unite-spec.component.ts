...
@Component({
  selector: 'app-recherche-par-unite-spec',
  templateUrl: './recherche-par-unite-spec.component.html',
  styleUrl: './recherche-par-unite-spec.component.css',
})
export class RechercheParUniteSpecComponent {
  searchTerm!: string;
  produits!: Produit[];
  ...
  onKeyUp(filterText: string) {
    this.produits = this.allProduits.filter((item) =>
      item.unitespec.toLowerCase().includes(filterText.toLowerCase())
    );
